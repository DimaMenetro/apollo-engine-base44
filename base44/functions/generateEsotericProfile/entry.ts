...
Deno.serve(async (req) => {
  try {
    ...
    // Only the creator or an admin may read/regenerate this subject's esoteric profile.
...
CURRENT DATE: ${today} — Year: ${currentYear}, Month: ${currentMonth}. Use ONLY this date for all transit, progression, and personal year calculations. Do NOT use any other year.${personalYearNote}${timeframe ? `\nTimeframe: ${timeframe}` : ''}${focus ? `\nFocus: ${focus}` : ''}${dspSummary ? `\nDSP Summary: ${dspSummary}` : ''}`;
    ...
    // ── ALL 4 CALLS in a SINGLE parallel batch ──
    // The four calls are mutually independent (they only share baseParams), so running
    // them in one Promise.all makes total wall-time ≈ the slowest single call instead of
    // slowest(A,B) + slowest(C,D). This halves execution time and keeps the synchronous
    // request comfortably under the gateway timeout that was causing 504 errors.
    const [rA, rB, rC, rD] = await Promise.all([
      ...
      // D: Limitation Statement + SME Validation
