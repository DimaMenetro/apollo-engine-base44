...
CRITICAL: ALL scores and probabilities MUST be integers on a 0-100 scale. No decimals.`,
  ...
  // Load DSP when it exists (fires after synthesis saves and query refetches)
  useEffect(() => {
    if (!subject?.dsp) return;
  ...
  // Auto-trigger synthesis only if analysis exists and no DSP yet
  useEffect(() => {
    if (!subject || isGenerating || subject.dsp) return;
    if (subject.analysis_results && !hasAutoGeneratedRef.current) {
