...
export default function SubjectReview() {
  ...
  const hasEnoughData = subject && (
    subject.analysis_results ||
    subject.stream_a_text?.length > 0 ||
    subject.stream_b_audio?.length > 0 ||
    subject.stream_c_video?.length > 0 ||
    subject.stream_d_behavioral?.length > 0 ||
    subject.stream_e_analog?.length > 0
  );

  const generateDraftDSP = async () => {
    if (!hasEnoughData) {
      setGenerateError('No data found. Please upload files and run Processing before generating the DSP.');
      return;
    }
    ...
    try {
      // Use analysis_results if available, otherwise fall back to raw file context
      const analysisContext = subject.analysis_results
        ? JSON.stringify(subject.analysis_results)
        : `No structured analysis available. Subject has uploaded files across streams: ${
            ['stream_a_text','stream_b_audio','stream_c_video','stream_d_behavioral','stream_e_analog']
              .filter(s => subject[s]?.length > 0)
              .join(', ')
          }. Generate the best possible profile from available context.`;
      const traitSchema = {
        type: "object",
        properties: {
          score: { type: "number" },
...
CRITICAL: ALL scores and probabilities MUST be integers on a 0-100 scale. No decimals.`,
  ...
  useEffect(() => {
    ...
    } else if (hasEnoughData && !hasAutoGeneratedRef.current) {
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 32 }}>
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          ...
          <button
            ...
            title={!hasEnoughData ? 'Upload files first' : 'Regenerate DSP'}
