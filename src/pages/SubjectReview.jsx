...
CRITICAL: ALL scores and probabilities MUST be integers on a 0-100 scale. No decimals.`,
  ...
  // Auto-trigger synthesis if analysis exists but no populated DSP
  useEffect(() => {
    if (!subject || isGenerating) return;
    if (isDspPopulated(subject.dsp)) return; // Already has real content
    if (subject.analysis_results && !hasAutoGeneratedRef.current) {
      hasAutoGeneratedRef.current = true;
      generateDraftDSP();
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [subject?.analysis_results, subject?.dsp]);
