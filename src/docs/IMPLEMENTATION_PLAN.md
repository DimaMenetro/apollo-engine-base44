...
| 4 — Honest stage-level UI | `UnifiedDossier` polls the active `DossierJob` and publishes "Synthesizing N/9: {section}" + percent to the EXISTING `BottomAccessory` glass slab via `AccessoryContext.updateProgress` (no new component — per operator directive, reuse the accessory built for this). Inline banner shows the same stage text. Failure messages carry the dying stage. | `pages/UnifiedDossier` |
