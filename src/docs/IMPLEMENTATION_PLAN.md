...
| 4 — Honest stage-level UI | `UnifiedDossier` polls the active `DossierJob` and publishes "Synthesizing N/9: {section}" + percent to the EXISTING `BottomAccessory` glass slab via `AccessoryContext.updateProgress` (no new component — per operator directive, reuse the accessory built for this). `BottomAccessory` now parses that string into a step-counter pill (`N/9`), the module name, a "Synthesizing · {subject}" subline, and a widened progress bar — granular per-stage rendering inside the same slab. Inline banner shows the same stage text. Failure messages carry the dying stage. | `pages/UnifiedDossier`, `components/ui/BottomAccessory` |
